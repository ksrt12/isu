...
const getElem = {
    ...
    input(id: string) {
        return this.id(id) as HTMLInputElement;
    },
    val(id: string) {
        return this.input(id).value;
    },
    selector(id: string) {
        return this.id(id) as HTMLSelectElement;
    },
    index(id: string) {
...
function checkBVIwoAgree() {
    ...
    for (const i of document.querySelectorAll("#report_rating_rep > tbody > tr")) {
        if (i.querySelector<HTMLTableElement>('td:nth-child(5)')!.innerText === 'без вступительных испытаний') {
            if (agree !== i.querySelector<HTMLTableElement>('td:nth-child(2)')!.innerText.substring(0, 8)) {
...
function getVSEROS(stream: string) {
    ...
    const vsosh: { [key: string]: boolean; } = { 'Математика': true };
    let subj: string[] = [];
    switch (stream) {
        case '01.03.02':
        case '09.03.01':
...
function loadEGEpoints() {
    ...
    for (const i of document.querySelectorAll("#report_baki_ege_rep > tbody > tr")) {
        EGE_points[i.querySelector<HTMLTableElement>('td:nth-child(2)')!.innerText] = Number(i.querySelector<HTMLTableElement>('td:nth-child(4)')!.innerText);
    }
    return EGE_points;
}
/**
...
function checkSTREAM() {
    const
        ...
        OLYMPSbyName = loadOLYMPS(),
        annul_text = 'Аннулировано ' + getElem.id('APPL_ANN').textContent,
        curr_stream = getElem.text('APPL_PROG').substring(0, 8),
        curr_olymp = getElem.text('APPL_OLYMP'),
        appl_usl = getElem.index('APPL_USL'),
        isBVI = (appl_usl === 1),
        isOlymps = (getElem.id('report_olymp_rep') !== null);
    let points: number | undefined, mess: string = "", err_mes: string | boolean = false,
        err_count = 0,
        warn_count = 0,
        isAnnul = (getElem.index('APPL_STATUS') === 1),
        minpoints = getMinPoints(curr_stream);

    const CountErr = (str: string) => {
        Err(str);
        err_count++;
    };
...
function checkAPPL() {
    ...
    if (HASH === "#appl") {
        ...
        document.querySelector<HTMLElement>("#appl_form > div.panel-body")!.onchange = addCheckButton("Проверить", "APPL_UPDATE", checkSTREAM);
    }
}
