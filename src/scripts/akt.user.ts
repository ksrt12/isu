...
window.addEventListener("load", () => {
    if (abit) {
        const prikaz = document.querySelector("body > div.main.page > section.static-page-rule > div > h1") as HTMLElement;
...
function initReport() {
    const json = loadJSON();
    if (json) {
        const input = document.querySelector("#report_list_filter > label > input") as HTMLInputElement;
...
function createReport(old_table: HTMLTableElement, json: IJson, keys: string[]) {
    ...
    if (isEnabled(next)) {
        ...
        console.groupEnd();
        clear.after(report);
        const new_keys = Array.from(merged_table.querySelectorAll<TD>("tbody > tr > td:nth-child(6)")).map(a => a.innerText);
        Log(`created! (${new_keys.length}/${keys.length})`, "white; font-weight: bold;");
        console.groupEnd();
        console.timeEnd("done");
        const diff = keys.filter((key: string) => !new_keys.includes(key));
...
function addList(delo_table: HTMLTableElement, data: IData, info: IMeta) {
    ...
    for (const i of isu_tbt) {
        ...
        const orig = (i.querySelector("td:nth-child(3)") as TD).innerText.includes("Да") ? "оригинал" : "";
        const isu_usl = (i.querySelector("td:nth-child(5)") as TD).innerText;
        if (data[fio] && !isu_usl.includes("Нет")) {
            ...
            const toTable = [
                'G' + stream, // направление
                usl, // условие || программа
                getID(fio_id), // ISU ID
                (i.querySelector("td:nth-child(1)") as TD).innerText, // номер дела
    ...
    const page = (document.querySelector("#report_list_paginate > ul > li.paginate_button.active") as TD).innerText;
...
function makeAktTable(hdr: number | string, grad: string): IResult {
    ...
    if (recomm) {
        for (const i of document.querySelectorAll<TR>("body > div.main.page.table-page > section.static-page-rule > div > div > table > tbody > tr:not(.hdr)")) {
            if (i.cells[2].className === "grn") {
                ...
                const fac = getFacBak(hdr as string);
                akt_data[fio] = createFullTable(tbody, akt_data, dubl, fio, fac, usl, hdr as string)!;
                count++;
            }
        }
    } else {
        for (const stream of document.querySelectorAll<HTMLElement>("body > div.main.page > section.static-page-rule > div > h3")) {
            const streamCode = stream.innerText.substring(0, 8);
            for (const i of stream.nextElementSibling!.querySelectorAll<TR>("tbody > tr:not(.hdr)")) {
...
function postgraduate(): IResult {
    ...
    for (const i of trs) {
        ...
        if (npr) {
            ...
            const getText = (cell: number) => i.cells[cell + shift].innerText;
            const getNum = (cell: number) => Number(getText(cell));
